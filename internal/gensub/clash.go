...
type Clash struct {
	...
	Tls     bool   `yaml:"tls"`
	Skip    bool   `yaml:"skip-cert-verify"`
}

func (kit *Clash) Content(proxy ProxyConfig) string {
	config := ClashYamlConfig{}
	for _, v2ray := range proxy.V2ray {
		config.Proxies = append(config.Proxies, Clash{
			Name:    v2ray.Name,
			Type:    "vmess",
			Server:  v2ray.Server,
			Port:    v2ray.Port,
			...
			Cipher:  v2ray.Cipher,
			Network: v2ray.Protocol,
			WsPath:  v2ray.WSPath,
			Tls:     v2ray.TLS,
			Skip:    v2ray.SkipCertVerify,
		})
		if config.ProxyGroup == nil {
			config.ProxyGroup = []ProxyGroup{{Name: "Proxy", Type: "select", Proxies: []string{v2ray.Name}}}
		} else {
			config.ProxyGroup[0].Proxies = append(config.ProxyGroup[0].Proxies, v2ray.Name)
		}
	}

	for _, ss := range proxy.SS {
		config.Proxies = append(config.Proxies, ClashSs{
			Name:     ss.Name,
			Type:     "ss",
			Server:   ss.Server,
			Port:     ss.Port,
		...
		if config.ProxyGroup == nil {
			config.ProxyGroup = []ProxyGroup{
				{Name: "Proxy", Type: "select", Proxies: []string{ss.Name}},
			}
		} else {
			config.ProxyGroup[0].Proxies = append(config.ProxyGroup[0].Proxies, ss.Name)
		}
	}
	defaultProxies := []string{"DIRECT", "Proxy"}
	config.ProxyGroup = append(config.ProxyGroup,
		ProxyGroup{
			Name:    "Domestic",
			Type:    "select",
	...
	if err != nil {
		fmt.Println("Clash.yaml marshal ", err)
	}

	buf, err := yamlTemplate.Execute(map[string]string{
		"ProxyContent": string(data),
	})
	if err != nil {
		fmt.Println("yaml template err ", err)
	}
	return buf
}

func (kit *Clash) Subscribe() string { return "" }

func (kit *Clash) URLSchema() string { return "" }

var yamlTemplate = tpl.Parse("yamlTemplate", `
# HTTP 代理端口
port: 7890
...
rules:
  ...
  # develop
  - IP-CIDR,10.120.0.0/16,Proxy

  # > iplclink
  - DOMAIN-KEYWORD,iplclink,Proxy
